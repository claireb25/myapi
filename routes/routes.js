...
var appRouter = function (app) {
  ...
  app.get("/formations", function (req, res) {
    dbconnect.query('SELECT dateStart.year as debut, dateEnd.year as fin, diploma_name as diplome, places.place_name as ecole, cities.city as ville, countries.country as pays FROM school INNER JOIN dates dateStart ON school.date_start_id = dateStart.id INNER JOIN dates dateEnd ON school.date_end_id = dateEnd.id INNER JOIN places ON school.place_id = places.id INNER JOIN cities ON places.city_id = cities.id INNER JOIN cities_countries ON cities.id = cities_countries.cities_id INNER JOIN countries ON countries.id = cities_countries.countries_id', function (error, results, fields) {
    //if error, print blank results
    if (error) {
      // console.log(error);
      var apiResult = {};
...
app.get("/competences", function (req, res) {
  dbconnect.query('SELECT skill FROM skills', function (error, results, fields) {
  //if error, print blank results
  if (error) {
    // console.log(error);
    var apiResult = {};
...
app.get("/touslesprojets", function (req, res) {
  dbconnect.query('SELECT project_name titre, images.picture as image FROM projects INNER JOIN images ON projects.main_img_id = images.id', function (error, results, fields) {
  //if error, print blank results
  if (error) {
    // console.log(error);
    var apiResult = {};
